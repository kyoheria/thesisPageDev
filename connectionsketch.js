...
function setup() {
  ...
  //changing position
  var ang = 2*PI/people.length;
  for(var i =1; i<people.length;i++){
    var preX = people[i-1].x;
    var preY = people[i-1].y;
    var alp = random(150,200);
    var x= cos(ang*i)*alp*2;
    var y = sin(ang*i)*alp;
    if(dist(preX,preY,x,y)<300){
      alp = random(150,400);
      x= cos(ang*i)*alp*2;
      y = sin(ang*i)*alp;
    }
    if(x>(width/2)-50){
      x = random(width/2-200,width/2);
    }else if(x<(width/2*-1)+150){
      x = random(-1*width/2+200,-1*width/2+150)
    }
    if(y>(height/2)-50){
      y=random(height/2-200,height/2);
    }else if(y<(height/2*-1)+100){
      y = random(-1*height/2+50,-1*height/2+150);
    }
    
    people[i].changeXY(x,y);
